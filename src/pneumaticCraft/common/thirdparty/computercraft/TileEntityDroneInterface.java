...
public class TileEntityDroneInterface extends TileEntity implements ITickable, IPeripheral, ManagedPeripheral,
        ...
        if(drone != null) {
            if(worldObj.isRemote) {
                ...
                if(ringSendList.size() > 0 && ringSendCooldown <= 0) {
                    ...
                    NetworkHandler.sendToDimension(new PacketSpawnRing(getPos().getX() + 0.5, getPos().getY() + 0.8, getPos().getZ() + 0.5, drone, ringSendList.remove(0)), worldObj.provider.getDimensionId());
                }
            }
        }
    }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt){
        Entity entity = worldObj.getEntityByID(pkt.getNbtCompound().getInteger("drone"));
    ...
    public TileEntityDroneInterface(){
        ...
        luaMethods.add(new LuaMethod("showArea"){
            ...
            public Object[] call(Object[] args) throws Exception{
                if(args.length == 0) {
                    ...
                    NetworkHandler.sendToDimension(new PacketShowArea(getPos(), area), worldObj.provider.getDimensionId());
                    return null;
                } else {
        ...
        luaMethods.add(new LuaMethod("hideArea"){
            ...
            public Object[] call(Object[] args) throws Exception{
                if(args.length == 0) {
                    NetworkHandler.sendToDimension(new PacketShowArea(getPos()), worldObj.provider.getDimensionId());
                    return null;
                } else {
                ...
                if(args.length == 1) {
                    ...
                    for(IProgWidget widget : WidgetRegistrator.registeredWidgets) {
                        if(widget.getWidgetString().equalsIgnoreCase(widgetName)) {
                            ...
                            messageToDrone(ItemDye.dyeColors[widget.getCraftingColorIndex()]);
                            curAction = widget;
                            return null;
                        }
                    }
    ...
    private void messageToDrone(Class<? extends IProgWidget> widget){
        messageToDrone(ItemDye.dyeColors[ItemProgrammingPuzzle.getWidgetForClass(widget).getCraftingColorIndex()]);
    }
