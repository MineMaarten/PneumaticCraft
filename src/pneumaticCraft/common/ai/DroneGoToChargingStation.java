...
public class DroneGoToChargingStation extends EntityAIBase{
    ...
    public boolean shouldExecute(){
        ...
        if(drone.getPressure(null) < PneumaticValues.DRONE_LOW_PRESSURE) {
            for(TileEntity te : drone.worldObj.loadedTileEntityList) {
        ...
        Collections.sort(validChargingStations, new Comparator(){
            ...
            public int compare(Object arg1, Object arg2){
                ...
                return Double.compare(PneumaticCraftUtils.distBetween(te1.getPos().getX(), te1.getPos().getY(), te1.getPos().getZ(), drone.posX, drone.posY, drone.posZ), PneumaticCraftUtils.distBetween(te2.getPos().getX(), te2.getPos().getY(), te2.getPos().getZ(), drone.posX, drone.posY, drone.posZ));
            }
        });

        for(TileEntityChargingStation station : validChargingStations) {
            boolean protect = PneumaticCraftUtils.getProtectingSecurityStations(drone.worldObj, station.getPos(), drone.getFakePlayer(), false, false) > 0;
            BlockPos pos = new BlockPos(station.getPos().getX(), station.getPos().getY(), station.getPos().getZ());
            if(protect) {
    ...
    public boolean continueExecuting(){
        if(curCharger.getUpgrades(ItemMachineUpgrade.UPGRADE_DISPENSER_DAMAGE) == 0 || curCharger.isInvalid()) {//If our path was blocked.
            ...
            if(isExecuting) {
                ...
                DroneClaimManager.getInstance(drone.worldObj).claim(new BlockPos(curCharger.getPos().getX(), curCharger.getPos().getY(), curCharger.getPos().getZ()));
            }
            return isExecuting;
        } else {
            chargingTime = 0;
